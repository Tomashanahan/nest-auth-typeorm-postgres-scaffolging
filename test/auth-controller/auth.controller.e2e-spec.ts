...
describe('AgroinsumoController (e2e)', () => {
  ...
  it('/user-form (Get)', async () => {
    ...
    expect(JSON.parse(user_form.text).agroinsumos.FuncionamientoAP).toBe(
      funcionamientoAP,
    );
    // USER
    expect(JSON.parse(user_form.text).agroinsumos.user.email).toBe(
      userRegister.email,
    );
    expect(JSON.parse(user_form.text).agroinsumos.user).toHaveProperty(
      'password',
    );
    expect(JSON.parse(user_form.text).agroinsumos.user).toHaveProperty(
      'fullName',
  ...
  it('/get-all-information (Get)', async () => {
    ...
    const token = JSON.parse(register.text).token;
    const funcionamientoAP =
      'Testing /get-all-information e2e FuncionamientoAP';
    const funcionamientoAP2 =
      'Testing 2 /get-all-information e2e FuncionamientoAP';
